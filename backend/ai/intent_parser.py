...
class TaskGroup(BaseModel):
    """One cabin × one set of routes × date ranges. Used for structured table imports."""
    ...
    date_ranges: list[DateRange] = Field(default_factory=list,
        description="Date ranges / periods for this group")
    specific_dates: list[str] = Field(default_factory=list,
        description="Specific dates if not using ranges")
    cabin: Literal["Y", "C", "F"] = Field("Y",
...
class SearchIntent(BaseModel):
    ...
    cabins: list[Literal["Y", "C", "F"]] = Field(default_factory=lambda: ["Y", "C"])
    currency: Literal["EUR", "GBP", "USD", "CNY", "HKD"] = "HKD"
...
- 用户粘贴数据里有 "2026-06-10 | 2026-06-17" 格式 → 识别为 date_ranges
...
典型场景：
- 单次往返（"6/10出发，7/10返回"）→ specific_dates=["2026-06-10"], return_dates=["2026-07-10"], task_type="batch"
- 多次往返（"6/10或6/17出发，7/10或7/17返回"）→ specific_dates=["2026-06-10","2026-06-17"], return_dates=["2026-07-10","2026-07-17"]
- 往返区间（"6月每天出发，7月某天返回"）→ date_start/date_end=出发区间, return_date_start/return_date_end=返程区间, task_type="single"

回复中说明：去程X个日期、回程X个日期，共N次查询（每方向独立计算）。
...
当用户粘贴包含如下格式的多行表格时（每行含 航线代码、日期段、类型、舱位 列）：
```
LON-BJS    2026-06-10 | 2026-06-17    DIRECT    经济舱
LON-CTU    2026-07-15 | 2026-07-22    I+D       公务舱
```

**立即进入表格解析模式**，不要询问任何澄清问题：

解析规则：
1. **航线格式**：`LON-BJS` → origin=LON, destination=BJS（用-分隔）
2. **日期段格式**：`2026-06-10 | 2026-06-17` → DateRange {{start:"2026-06-10", end:"2026-06-17"}}
3. **类型列**：DIRECT/I+D/I+I 仅作参考标签，不影响搜索逻辑，忽略即可
4. **按舱位分组**：同一舱位的所有行合并为一个 TaskGroup
5. **舱位映射**：经济舱→Y，公务舱/商务舱→C，头等舱→F
6. 每个 TaskGroup 的 `origins` = 该舱位下所有出发地（去重）
7. 每个 TaskGroup 的 `destinations` = 该舱位下所有目的地（去重，按出现顺序）
8. 每个 TaskGroup 的 `date_ranges` = 所有唯一日期段（去重）
9. `label` = 例如 "经济舱（28条航线）" 或 "公务舱（3条航线）"
10. task_groups非空时：origins/destinations/specific_dates/date_ranges 字段留空列表
11. `ready_to_search=true`，`task_type="batch"`
12. `reply` 简述："已识别X组搜索任务（经济舱N条航线、公务舱M条航线），4个时间段，合计约K次查询"
13. **不要问任何问题**，直接返回完整 task_groups

示例输出（针对上述格式）：
- task_groups=[{{origins:["LON"], destinations:["BJS","CTU",...], date_ranges:[...], cabin:"Y", label:"经济舱（28条）"}}, {{origins:["LON"], destinations:["BJS","CTU","PVG"], date_ranges:[...], cabin:"C", label:"公务舱（3条）"}}]
